...
type ProjectManager(checker: FSharpChecker) =
    ...
    let printList (l: string list) =
        l |> List.iter (dprintfn "    - %s")
    ...
    let printOptions(options: FSharpProjectOptions) =
        ...
        dprintfn "  SourceFiles:"
        printList (List.ofArray options.SourceFiles)
        dprintfn "  ReferencedProjects:"
        printList [for dll, _ in options.ReferencedProjects do yield dll]
        dprintfn "  OtherOptions:"
        printList (List.ofArray options.OtherOptions)
    ...
    let rec analyzeLater(fsprojOrFsx: FileInfo): LazyProject =
        ...
        let analyzeFsx(fsx: FileInfo) =
            ...
            let inferred, errors =
                checker.GetProjectOptionsFromScript(fsx.FullName,
                                                    source,
                                                    loadedTimeStamp=fsx.LastWriteTime,
                                                    assumeDotNetFramework=false)
                |> Async.RunSynchronously
            // Substitute references from the running framework (in fslsFrameworkPath) with
            // references in systesmFramework, but only if they actually exist there. There are
            // extra assemblies FSLS ships with that aren't in the base framework.
            let refArgExists(arg: string) =
                File.Exists(arg.Replace("-r:", ""))
            // Bypass ASP.NET Core framework since it lacks most of the assemblies of the App
            // framework, and find the most up-to-date version of .NET Core to get assemblies
            // from.
            let systemFramework =
                ProjectCracker.findRuntimePaths ()
                |> Seq.filter (fun runtime -> runtime.path.Contains("Microsoft.NETCore.App"))
                |> Seq.sortByDescending (fun runtime -> (runtime.majorVersion, runtime.minorVersion))
                |> Seq.head
            let fslsFrameworkPath =
                Path.GetDirectoryName(typeof<System.Object>.Assembly.Location)
            let matchesName (o:string) (f: string) =
                o.EndsWith(f)
            let referencedPaths =
                [for p in dotNetFramework do
                 if File.Exists(Path.Combine(systemFramework.path, p)) then
                     yield "-r:" + Path.Combine(systemFramework.path, p)]
                @ [for o in inferred.OtherOptions do
                   let alreadyIncluded = List.exists (matchesName(o)) dotNetFramework
                   if o.Contains("-r:") then
                       if not(alreadyIncluded) then
                          let systemAssembly = o.Replace(fslsFrameworkPath, systemFramework.path)
                          if refArgExists(systemAssembly) then
                              yield systemAssembly
                   else
                       yield o]

            let combinedOtherOptions = List.toArray referencedPaths
